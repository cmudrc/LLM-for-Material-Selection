...
def prompt_response(design_choice: str, criterion: str):
    ...
    Suggest the optimal material that satisfies the case above?

    As a materials science and design engineer with experience in this field, 
    you are supposed to suggest the most optimal material which satisfies {criterion} for the {design_choice}.
    The suggested material should be a string.
    ...
    'design' : {design_choice},
    'criterion' : {criterion},
    'suggested material' : suggested material as a string,
    )
    """
...
for j in range(len(design_choice)):
    for k in range(len(criterion)):
        suggest_json = prompt_response(design_choice[j], criterion[k])

#         print(f"Output {j}",score_json)

        path = f"/Users/yashpatawari/LLM_Materials/Top_Suggest/{design_choice[j]}_{criterion[k]}_Top"

        with open(path, "w") as json_file:
            json.dump(suggest_json, json_file)
